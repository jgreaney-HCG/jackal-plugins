...
After discovering phases, check if `.jackal/implementation-plan-guidance.md` exists in the project root:

```bash
# Check for implementation guidance (note the absolute path for later use)
ls [project-root]/.jackal/implementation-plan-guidance.md
```
...
- IMPLEMENTATION_GUIDANCE: absolute path to `.jackal/implementation-plan-guidance.md` (**only if it exists**—omit entirely if the file doesn't exist)
...
After all phases complete, invoke the `jackal-extending-claude:project-claude-librarian` subagent (when available) to review changes and update CLAUDE.md files if needed.

```
<invoke name="Task">
<parameter name="subagent_type">jackal-extending-claude:project-claude-librarian</parameter>
...
<parameter name="prompt">
  ...
  Follow the jackal-extending-claude:maintaining-project-context skill to:
...
**If librarian subagent is unavailable:** skip this entire step. Say aloud that you're skipping it because the `jackal-extending-claude` plugin is not available.
