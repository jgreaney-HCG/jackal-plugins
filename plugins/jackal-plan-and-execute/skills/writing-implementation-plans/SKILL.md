...
Before creating the Finalization task, check if `.jackal/implementation-plan-guidance.md` exists. If it does, include its absolute path in the task description:

```markdown
# If .jackal/implementation-plan-guidance.md exists:
- [ ] Finalization: Run code-reviewer over all phase files (guidance: [absolute path to .jackal/implementation-plan-guidance.md]), fix ALL issues including minor ones
      → blocked by: all Phase *D tasks

# If .jackal/implementation-plan-guidance.md does NOT exist:
...
<parameter name="prompt">
  ...
  IMPLEMENTATION_GUIDANCE: [absolute path to .jackal/implementation-plan-guidance.md, or "None" if file does not exist]
...
```
<invoke name="Task">
<parameter name="subagent_type">jackal-basic-agents:opus-general-purpose</parameter>
