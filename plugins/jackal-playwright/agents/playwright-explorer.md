...
Unless given other directions, these tools come from the `jackal-playwright-internal` MCP server configured in `.mcp.json`. If directed to use a different MCP server, use those tools instead.
