---
description: "EXPERIMENTAL: Execute implementation plan with just-in-time loading and task markers"
argument-hint: [absolute-plan-dir] [absolute-working-dir]
---

# Execute Implementation Plan (Experimental)

**Implementation plan directory:** `$1`
**Working directory:** `$2`

This is the EXPERIMENTAL execution workflow that uses:
- Just-in-time phase loading (reads one phase at a time)
- Task/subcomponent markers for efficient extraction
- Context windows for subagents (task + adjacent tasks)
...
2. **Engage the skill:** Use your Skill tool to invoke `executing-an-implementation-plan-experimental`
3. **When the skill asks for a plan path:** The user has already provided it: `$1`. Do not ask again.

The skill should execute all phases in the plan directory using the experimental just-in-time loading workflow. Follow it exactly as written.
